...
const templateData = [
  {
    ...
    "Max Team Size": 5,
    "Max Teams": 2,
    "Professor Email": "prof.smith@university.edu",
    Department: "Computer Science",
    "Application Deadline": "2024-03-15",
    "Project Start Date": "2024-03-22",
    "Project End Date": "2024-05-17",
    Budget: 500,
  ...
  },
  {
    ...
    "Max Team Size": 6,
    "Max Teams": 3,
    "Professor Email": "prof.johnson@university.edu",
    Department: "Software Engineering",
    "Application Deadline": "2024-03-20",
    "Project Start Date": "2024-03-27",
    "Project End Date": "2024-06-05",
    Budget: 0,
    Prerequisites: "JavaScript, Basic mobile development",
  },
]

export function ExcelUploadSection({ onProjectsUpdate }: ExcelUploadSectionProps) {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        const project: BulkProjectData = {
          ...
          technologies: getVal("Technologies")?.split(";").map((t) => t.trim()) || [],
          maxTeams: Number.parseInt(getVal("Max Teams")) || 1,
          supervisorEmail: getVal("Supervisor Email"),
          coSupervisorEmail: getVal("CoSupervisor Email"),
        ...
        if (!project.title || !project.description || !project.supervisorEmail || !project.coSupervisorEmail) {
  ...
  const onDrop = useCallback(
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        const projects = parseCSVData(text)

        // Validate projects
        const invalidProjects = projects.filter((p) => !p.title || !p.description || !p.supervisorEmail)
        if (invalidProjects.length > 0) {
          throw new Error(`${invalidProjects.length} projects are missing required fields`)
        }
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {uploadStatus === "uploading" ? (
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">Parsed {parsedData.length} projects</p>
            </div>
          ) : uploadStatus === "error" ? (
            <div>
            ...
            <div>
              <p className="text-muted-foreground mb-2">Drag & drop your CSV/Excel file here, or click to browse</p>
              <p className="text-xs text-muted-foreground">Supports .csv and .xlsx files up to 5MB</p>
            </div>
          )}
        </div>
      </div>
      ...
      {uploadStatus === "success" && parsedData.length > 0 && (
        <Alert>
          ...
          <AlertDescription>
            Successfully parsed {parsedData.length} projects. Review them in the preview section below.
          </AlertDescription>
        </Alert>
      )}
      ...
      {uploadStatus === "error" && (
        <Alert variant="destructive">
          ...
          <AlertDescription>{errorMessage}. Please check your file format and try again.</AlertDescription>
        </Alert>
      )}
